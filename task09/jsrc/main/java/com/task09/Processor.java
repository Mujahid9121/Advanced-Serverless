...
public class Processor implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayV2HTTPResponse> {
	...
	private final String tableDB = System.getenv("target_table");
	...
	public APIGatewayV2HTTPResponse handleRequest(APIGatewayProxyRequestEvent request, Context context) {
		...
		try {
			...
			Table table = dynamoDB.getTable(tableDB);
			Item item = new Item()
			...
			response = APIGatewayV2HTTPResponse.builder()
					.withStatusCode(200)
					.withBody("Weather forecast has been successfully updated in the database.")
					.build();
			...
			response = APIGatewayV2HTTPResponse.builder()
					.withStatusCode(500)
					.withBody("{\"statusCode\": 500, \"message\": \"The request could not be processed due to a server error. Error details: " + ex.getMessage() + "\"}")
					.build();
		}
